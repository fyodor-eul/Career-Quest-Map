...
def ui_results_to_engine_answers(ui_results: List[Dict[str, Any]], *, id_prefix: str = "q", start_index: int = 1) -> List[Dict[str, Any]]:

    out: List[Dict[str, Any]] = []

    for i, ui in enumerate(ui_results, start=start_index):
        ui_type = ui.get("type")
        prompt = ui.get("question", "")
        ...
        if ui_type == "multiple_choice":
            engine_type = "mcq"
            opts = ui.get("answers", []) or []
            idx = int(ui.get("user_choice_index", 0))
            answer = opts[idx] if 0 <= idx < len(opts) else ""

        elif ui_type == "slider":
            engine_type = "slider"
            answer = int(ui.get("user_choice_index", 0))

        elif ui_type == "textinput":
            engine_type = "text"
            answer = str(ui.get("user_input", "")).strip()

        else:
            # Unknown UI type — keep it safe
            engine_type = "text"
            answer = ""
...
"""
def main() -> None:
    global quiz_questions_home, quiz_questions_wiseman
    cfg = AppConfig()
    llm = LLMClient(
        cfg.azure_endpoint,
        cfg.azure_api_key,
    ...
    print(type(quiz_questions_home))

    # Mock answers for part 1 to drive part 2
    #part1_answers = _mock_answers_from_payload(part1_payload)

    # Real answers 
    part1_answers = ui_results_to_engine_answers(quiz_questions_home)

    print(part1_answers)
    print(type(part1_answers))
...
def get_question_part1():
    global quiz_questions_home
...
def get_question_part2(part1_answers):
    global quiz_questions_wiseman
    ...
    return part2_payload

def submit_part2_answers(part2_payload, part2_answers):
    inferred_fields = part2_payload.get("inferred_fields", [])
    poly_path_choice = "Work" if education_status == "Poly" else None
...
def main():
    get_question_part1()
    part1_answers = ui_results_to_engine_answers(quiz_questions_home)
    part2_payload = get_question_part2(part1_answers)
    part2_answers = ui_results_to_engine_answers(quiz_questions_wiseman)
    submit_part2_answers(part2_payload, part2_answers)
